...
with open('text.txt', 'r', encoding='utf-8') as file:
    with open('stats.txt', 'w') as stats_file:
        text = file.read()
        stats_file.write(f'Статистика по файлу:\n\nКол-во символов с пробелами: {len(text)}\n')  # кол-во символов
        stats_file.write(f'Кол-во символов без пробелов: {len(text.replace(" ", ""))}\n')  # кол-во символов без пробелов

        file.seek(0)
        lines = len(file.readlines())  # кол-во строк
        stats_file.write(f'Кол-во строк: {lines}\n')
        lines_2 = text.count('\n') + 1  # второй способ реализации кол-ва символов через текст файла
